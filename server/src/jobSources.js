...
export async function fetchJobs(sourceUrl,atsType){
 ...
 // Career pages may render job cards with JavaScript while keeping the ATS URL in the raw HTML.
 const discoveredAts=[];const greenhouseMatches=html.match(/https?:\\/\\/boards\\.greenhouse\\.io\\/[^"'<>\\s]+/gi)||[];const leverMatches=html.match(/https?:\\/\\/jobs\\.lever\\.co\\/[^"'<>\\s]+/gi)||[];const workdayMatches=html.match(/https?:\\/\\/[^"'<>\\s]+\\.myworkdayjobs\\.com\\/[^"'<>\\s]+/gi)||[];discoveredAts.push(...greenhouseMatches,...leverMatches,...workdayMatches);
 for(const href of [...new Set(discoveredAts)].slice(0,20)){try{const u=new URL(href.replace(/&amp;/g,"&"));let type=null;if(u.hostname.endsWith("greenhouse.io"))type="greenhouse";else if(u.hostname==="jobs.lever.co")type="lever";else if(u.hostname.endsWith("myworkdayjobs.com"))type="workday";if(type){const found=await fetchJobs(u.toString(),type);if(found.length)return found}}catch{}}
