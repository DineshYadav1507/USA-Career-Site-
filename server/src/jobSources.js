...
export async function fetchJobs(sourceUrl,atsType){
 ...
 const $=cheerio.load(html),links=[];$("a[href]").each((_,a)=>{const title=$(a).text().replace(/\s+/g," ").trim(),href=absolute(sourceUrl,$(a).attr("href"));if(href&&/(careers?|jobs?|openings?|opportunities?)/i.test(title+" "+href))links.push({title,href})});
 const candidates=[...new Map(links.map(x=>[x.href,x])).values()].slice(0,12);const out=[];
