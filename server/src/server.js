...
async function saveJobs(source,jobs){
 ...
 for(const j of jobs){
  ...
  VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,NOW(),NOW(),?,1)
  ON DUPLICATE KEY UPDATE company_id=VALUES(company_id),title=VALUES(title),description=IF(VALUES(description)<>'' ,VALUES(description),description),location=IF(VALUES(location)<>'' ,VALUES(location),location),location_type=VALUES(location_type),experience_level=VALUES(experience_level),category=VALUES(category),employment_type=VALUES(employment_type),skills_json=VALUES(skills_json),apply_url=VALUES(apply_url),source_job_url=VALUES(source_job_url),posted_at=VALUES(posted_at),date_source=VALUES(date_source),last_seen_at=NOW(),expires_at=VALUES(expires_at),is_active=1,updated_at=NOW()`;
...
app.post("/api/admin/sources/import-excel",adminAuth,upload.single("file"),async(req,res)=>{try{if(!req.file)return res.status(400).json({error:"Excel file is required"});const result=await importCareerWorkbook(req.file.buffer,{adminId:req.admin.id,fileName:req.file.originalname,pool,upsertSource,scanSource});res.json({ok:true,...result})}catch(e){res.status(400).json({error:e.message})}});
...
app.get("/api/admin/users/:id/master-cv",adminAuth,async(req,res)=>{const [rows]=await pool.query("SELECT id,name,email,master_cv_name,master_cv_text,master_cv_updated_at FROM users WHERE id=? LIMIT 1",[req.params.id]);if(!rows[0])return res.status(404).json({error:"User not found"});res.json({cv:rows[0]})});
